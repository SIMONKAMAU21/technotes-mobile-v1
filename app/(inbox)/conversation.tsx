...
let conversationId = null;
  ...
  let receiverPhoto;
  if (
    conversation?.lastMessage?.receiverId?._id === userId  || userData?._id !== userId ||
    conversation?.lastMessage?.senderId._id === userId
  ) {
    title =
      conversation?.lastMessage?.senderId?.name || userData?.name ||
      conversation?.lastMessage?.receiverId?.name;
    receiverPhoto =
      conversation?.lastMessage?.senderId?.photo || userData?.photo ||
  ...
  const receiverId = getReceiverIdFromConversation(conversationId, userId) || userData?._id;
