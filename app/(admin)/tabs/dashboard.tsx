      ...
      >
        ...
        <View className={`rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4 shadow-sm mb-4`}>
          ...
          <View className="flex-row flex-wrap gap-4 justify-between mt-4">
            <Card color="white" icon="people" count={adminCount} bgColor="bg-primary" label="Admins" />
            <Card color="white" icon="book" count={studentCount} bgColor="bg-secondary" label="Students" />
            <Card color="white" icon="school" count={teacherCount} bgColor="bg-secondary" label="Teachers" />
            <Card color="white" icon="person" count={parentCount} bgColor="bg-primary" label="Parents" />
          </View>
        </View>
        ...
        <View className={`rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4 shadow-sm mb-4`}>
          ...
          <View className="space-y-4 flex-row justify-between">
            <View className="bg-blue-100 p-4 rounded-lg">
              ...
              <Text className="text-blue-600 text-2xl font-bold mt-1">{users.length}</Text>
            </View>
            ...
            <View className="bg-purple-100 p-4 rounded-lg">
              <Text className="text-purple-800 font-semibold">Regular Users</Text>
              <Text className="text-purple-600 text-2xl font-bold mt-1">
                {users.filter(user => user.role === 'student' || user.role === 'teacher' || user.role === 'parent').length}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View className="flex-row justify-between">
          <View className="bg-white p-4 rounded-lg w-full">
            ...
            {users.slice(0, 4 ).map((user: {name: string; role: string,photo:string}, index: number) => (
              <View key={index} className="flex-row items-center justify-between py-2 border-b border-gray-200">
                <View className="flex-row items-center gap-2">
                  {user?.photo ? (
                    <Image source={{uri: user?.photo}}  alt="user" className="w-10 h-10 rounded-full" />
                  ) : (
